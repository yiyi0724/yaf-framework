    ...
    $(function() {
        (function(win) {
            ...
            var resizeFun = function() {
                ...
                $('#rewardFrameWrap').width(width).height(h);
                $('#rewardFrame').width(width).height(h);
                //重置辅助按钮位置
                $('#assistiveTouch').css({
                    right: '0px',
                    top: '140px',
        ...
        (function() {
            ...
            //window.frames[0]等价于document.getElementById('myIframe').contentWindow
            ...
            function receiveMsg(e) {
                if (e.source !== myIframe) return; //检查来源地址
                var dataObj = JSON.parse(e.data);
                var method = dataObj['method'];
                var choosePay = dataObj['params'];
                if (method == 'pay' && choosePay) {
                    choosePay['success'] = function(res) {
                        console.log('iframeChoosePaySuccess:');
                        console.log(res);
                        ...
                        $('#myIframe').contents().find('body').append(waitingPayResultHtml);
                        /**
                         * 每隔1s发送请求，查询是否已付款
                         */
                    ...
                    wx.chooseWXPay(choosePay); //调用微信支付
                }
            }
        })();
        ...
        (function() {
            ...
            var $redpack = $('#redpack');
            var isActivity = parseInt($('#isActivity').val(), 10);
            ...
            window.manyouSidebar = {
                ...
                show: function() {
                    ...
                    //window.manyouSidebar.resizeDialog();
                    window.manyouSidebar.getSidebarInfo();
                    $sidebar.fadeIn(200);
                    $sidebar.find('.modal-dialog').animate({
                        "margin-left": "0px"
                    }, 200, function() {
                        //window.manyouChat.connect()
                    });
                },
                hide: function() {
                    /**
                ...
                resizeDialog: function() {
                     ...
                     * 重置聊天室高度
                     */
                    $sidebar.find('.dialog-box').height($(window).height() - 104);
                },
                ...
                getSidebarInfo: function() {
                    ...
                    $.ajax({
                        ...
                        .done(function(result) {
                            ...
                            if (result.status) {
                                $('#onlineTime').html(data.muinute);
                                if (data.sign) {
                                    $redpack.find('.sign').addClass('signed').find('a').html('已签到');
                                }
                            }
                        });
                },
                countOnlineTime: function() {
                    /**
                     * 更新在线时长
                     */
                    if (!isActivity) return;
                ...
                showRewardFrame: function(url) {
                    ...
                    rewardFrameWrap.style.width = w + 'px';
        ...
        (function() {
            ...
            window.manyouChat = {
                connect: function() {
                    if (window.isDebug) {
                        window.manyouChat.Config = {
                            chatAppKey: "x4vkb1qpvsrkk",
                            chatToken: "hy07iZAUFloEjozpvstMg/kCNayJdmr1w+5mjqkz9SXdrnEI+ZaihDKa/ke2SxSaEFY3+J/nEntxJSvEY7SYWw==",
                            chatRoomId: "test_1"
                        };
                        window.manyouChat.registerSend();
                        //window.manyouChat._rongIMConnect();
                    } else {
                        $.getJSON("/auth/chat?gameId=" + 'window.HORTOR_GAME_CONF.GameId' + "&userId=" + 'window.HORTOR_CONF.UserId', function(result) {
                            window.manyouChat.Config = result;
                            window.manyouChat.Config.chatRoomId = 'window.HORTOR_GAME_CONF.GameId' + '_' + window.manyouChat.Config.chatRoomCnt;
                            window.manyouChat.registerSend();
                            //window.manyouChat._rongIMConnect();
                        });
                    }
                ...
                handleSend: function() {
                    /*var mainContentVal = $mainContent.val();
                    if (mainContentVal != '') {
                        if (window.manyouChat.leastSendTime != 0 && new Date().getTime() - window.manyouChat.leastSendTime < 3000) {
                            return alert("请不要频繁发送消息! ");
                        }
                        if (mainContentVal.length > 40) {
                            return alert("聊天内容不能超过40个字！");
                        }
                        $mainContent.val("").focus();
                    }*/
                }
            };
        })();
